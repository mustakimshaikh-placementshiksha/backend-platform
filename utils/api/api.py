...
# =========================
# Base API View (Swagger Compatible)
# =========================
class APIView(DRFAPIView):
    """
    Refactored by: Mustakim.shaikh@placementshiksha.com

    Custom APIView (Refactored for Swagger compatibility)
    Inherits from rest_framework.views.APIView to allow introspection by drf-yasg.

    Logic Flow:
    - Provides standardized response methods (`success`, `error`).
    - Implements `invalid_serializer` to handle validation errors uniformly.
    - Overrides `dispatch` to handle custom `APIError` exceptions.
    """
    ...
    def dispatch(self, request, *args, **kwargs):
        """
        Overriding dispatch to maintain custom exception handling
        """
        try:
            ...
            logger.exception(e)
            return self.error(msg=str(e), err="server-error")
...
# =========================
# Serializer Validation Decorator
# =========================
def validate_serializer(serializer):
    """
    Refactored by: Mustakim.shaikh@placementshiksha.com
    ...
    Logic Flow:
    1. Instantiates the serializer with `request.data`.
    2. If valid, attaches the serializer and valid data to the request object.
    3. If invalid, returns a standardized error response via `invalid_serializer`.
    ...
    def validate(view_method):
        ...
        def handle(*args, **kwargs):
            ...
            s = serializer(data=request.data)
            if s.is_valid():
                # request.data is a property and cannot be set directly in newer DRF versions
